...
public class Entrega6 {
    ...
    public static void main(String[] args) {
        ...
        resultado = GaussPivoteEscalonado(coefs); 
    ...
    public static ArrayList<Double> GaussPivoteEscalonado (ArrayList<ArrayList<Double>> coefs)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            for(int j = 0; j <= n; j++)
            {
                max = abs(coefs.get(i).get(j));
                if (s[i] < max)
                    s[i] = max;
            }
            for (p = i; p < n; p++)
            {
                if (abs(coefs.get(p).get(i))/s[p] > abs(coefs.get(findP).get(i))/s[findP])
                {
                    findP = p;
                }
            }
